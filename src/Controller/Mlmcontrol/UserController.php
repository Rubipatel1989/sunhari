...
class UserController extends AppController
{
    public function login()
    {
        ...
        if ($this->request->is("post")) {
            /*echo '<pre>';
            print_r($this->request->getData('User'));exit;*/
            ...
            $userInfo = $usersTable
                ->find("all", [
                    "conditions" => [
                        ...
                        "Users.role_id !=" => 2,
                        "Users.status" => 1,
                        "Users.is_blocked IS NULL",
                    ],
                ])
                ->first();

            if (!empty($userInfo)) {
                $this->request->getSession()->write("adminUserId", $userInfo->id);

                return $this->redirect($this->backend_url.'/user/dashboard');
            } else {
                $this->Flash->error(
                    __("Entered username or password is wrong.")
                );
            }
        }
    }
    ...
    public function dashboard()
    {
        ...
        //print_r($this->adminUserId);exit;

        $title = $prefix_title . " Dashboard";
        ...
        $join = [
            ...
            [
                ...
                "conditions" => ["Payer.id = Wallets.transfer_by"],
            ],
        ];
        ...
        $adminStatistics = $usersTable->getAdminStatistics();

        $this->set("adminStatistics", $adminStatistics);

        $conditions = [
            "Fundrequests.status" => 0,
        ];
        ...
        $join = [
            ...
            [
                ...
                "conditions" => [
                    'Attachments.reference_id = Users.id and Attachments.reference_type = "profile_photo"',
                ],
            ],
        ];

        $group = [" Fundrequests.id"];
        ...
        $totalPaymentRequest = $fundrequestsTable
            ->find("all", [
                "fields" => $fields,
                "conditions" => $conditions,
                "order" => $order,
        ...
        $this->set("totalPaymentRequest", $totalPaymentRequest);

        $conditions = ["Tickets.status" => 0];
        ...
        $this->set("totalTickets", $totalTickets);
    }
    ...
    public function accountPassword()
    {
        ...
        if ($this->request->is("post")) {
            ...
            if (isset($postData["btn_account_password"])) {
                $userInfo = $usersTable
                    ->find("all", [
                        "conditions" => [
                            "Users.id" => $user_id,
                ...
                if (!empty($userInfo)) {
                    ...
                    if ($usersTable->save($userData)) {
                        ...
                        return $this->redirect($this->backend_url.'/user/account_password');
                    }
                } else {
                    $this->Flash->error(
            ...
            if (isset($this->request->getData()["btn_account_password"])) {
                ...
                if (!empty($userInfo)) {
                    ...
                    if ($usersTable->save($userData)) {
                        ...
                        return $this->redirect($this->backend_url.'/user/transaction_password');
                    }
                } else {
                    $this->Flash->error(
                        __(
    ...
    public function logout()
    {
        ...
        $this->request->getSession()->delete("adminUserId");
    ...
    public function upgrade() {
        ...
        $title = $prefix_title.' User : Upgrade';
        ...
        $packages = $packagesTable->find('all', array('conditions' => array('Packages.status' => 1)))->enableAutoFields(true)->toArray();
        ...
        if($this->request->is('post')){
            ...
            exit;*/
            if(!empty($this->request->getData()['Upgrade']['package_id']) && $this->request->getData()['Upgrade']['upgraded_id']){

                $package = $packagesTable->find('all', array('conditions' => array('Packages.id' => $this->request->getData()['Upgrade']['package_id'])))->enableAutoFields(true)->first();
                if(!empty($package)){
                    $user_id                    =   $this->adminUser->id;
                    ...
                    $upgrade->package_bv        =   $package->package_bv;
                    ...
                    if($upgradesTable->save($upgrade)){
                       ...
                       if($usersTable->save($userInfo)){
                            $conditions = array('Users.id' => $upgraded_id);
                            $join = array(
                                            array(
                                                'table' => 'details',
                                                'alias' => 'Details',
                            ...
                            $fields = array('Details.id', 'Details.first_name', 'Details.middle_name', 'Details.last_name', 'Details.contact_no');
                            $userData = $usersTable->find('all', array('fields' => $fields, 'conditions' => $conditions, 'join' => $join))->enableAutoFields(true)->first();
                            $usersTable->updateParentsOnUpgrade($upgraded_id, $userData->parent_id, $package->package_bv);
                        
                            if($userData->sponsor_id > 0 && !empty($userData->sponsor_id) && isset($userData->sponsor_id)){
                                $sponsorInfo = $usersTable->find('all', array('conditions' => array('Users.id' => $userData->sponsor_id)))->enableAutoFields(true)->first();
                                ...
                                if($userData->position == 'left'){
                                    $sponsor->total_direct_acitve_left = $sponsorInfo->total_direct_acitve_left + $package->package_bv;
                                }else{
                                    $sponsor->total_direct_acitve_right = $sponsorInfo->total_direct_acitve_right + $package->package_bv;
                                }
                                ...
                                if ($package->package_amount >= 10000) {
                                    $conditions = [
                                        "AutoPools.user_id" => $upgraded_id,
                                    ];

                                    $autoPool = $autoPoolsTable
                                        ->find("all", [
                                            "conditions" => $conditions,
                                        ])->first();
                                    if (!$autoPool) {
                                        $autoPoolData = $autoPoolsTable->newEmptyEntity();
                                        $autoPoolData->user_id  = $upgraded_id;
                                        $autoPoolsTable->save($autoPoolData);
                                    }
                                }

                                $commission = $commissionsTable->find('all', array('conditions' => array('Commissions.status' => 1)))->enableAutoFields(true)->first();

                                //$checkUserLabelUp = $usersTable->checkUserLabelUp($userData->sponsor_id, $package->booster_time);

                                $direct_amount = $package->direct_amount;
                                /*if($checkUserLabelUp > 0){
                                    $direct_amount = $package->booster_amount;
                                }*/
                            ...
                            $this->Flash->success(__('User has been upgraded successfully.'));

                            return $this->redirect($this->backend_url.'/users/upgrade-history');

                       }
                    }

                }else{
                    $this->Flash->error(__('Invalid Package.'));
                }
            }
        }
    ...
    public function degrade()
    {
        ...
        if ($this->request->is("post")) {
            ...
            exit;*/
            $postData = $this->request->getData();
            if (isset($postData["Upgrade"]["upgraded_id"])) {
                $user_id = $this->adminUser->id;
                $upgraded_id = $postData["Upgrade"]["upgraded_id"];
                $conditions = [
                ...
                if (!empty($upgradeInfo)) {
                    $package = $packagesTable
                        ->find("all", [
                            "conditions" => [
                                "Packages.id" => $upgradeInfo->package_id,
                            ],
                        ])
                        ->enableAutoFields(true)
                        ->first();

                    $upgradesTable->deleteAll(["upgraded_id" => $upgraded_id]);
                    ...
                    if ($usersTable->save($userInfo)) {
                        ...
                        //$usersTable->updateParentsOnDegrade($upgraded_id, $userData->parent_id, $package_bv);

                        $usersTable->updateParentsOnPackageDegrade(
                            $upgraded_id,
                            $userData->parent_id,
                            $package_bv
                        );
                        ...
                        if (
                            ...
                            if ($userData->position == "left") {
                                $sponsor->direct_active_left_one =
                                    $sponsorInfo->direct_active_left_one -
                                    $package_bv;
                            } else {
                                $sponsor->direct_active_right_one =
                                    $sponsorInfo->direct_active_right_one -
                                    $package_bv;
                            }
                            ...
                            $downlines = $downlinesTable
                                ->find("all", [
                                    "conditions" => [
                                        "Downlines.user_table_id" => $upgraded_id,
                                    ],
                                ])
                                ->enableAutoFields(true)
                                ->toArray();
                            ...
                            foreach ($downlines as $downline) {
                                $downlineInfo = $downlinesTable->get(
                                    $downline->id
                                );

                                $downlineInfo->total_join =
                                    $downline->total_join -
                                    $package->package_bv;
                        ...
                        $achievedRewardsTable->deleteAll([
                            "upgraded_id" => $upgraded_id,
                        ]);

                        $achievedBonanzasTable->deleteAll([
                            "upgraded_id" => $upgraded_id,
                        ]);
                    }

                    $this->Flash->success(
                        __("User has been degraded successfully.")
                    );
    ...
    public function editAccount($intUserId = null)
    {
        if (!$this->request->getSession()->check("adminUserId")) {
            return $this->redirect($this->backend_url.'/user/login');
        }
        ...
        $title = $prefix_title . " User : Edit Account";
        ...
        if ($this->request->is("post")) {
            ...
            if (
                !empty($this->request->getData()["User"]["id"]) ||
                !empty($this->request->getData()["User"]["password"])
            ) {
                $checkUsername = $usersTable
                    ->find("all", [
                        "conditions" => [
                            "Users.id" => $this->request->getData()["User"]["id"],
                        ],
                    ])
                    ->count();
                ...
                if ($checkUsername > 0) {
                    $userSaveData = $usersTable->get(
                        $this->request->getData()["User"]["id"]
                    );

                    $userSaveData->password = md5(
                        $this->request->getData()["User"]["password"]
                    );
                    ...
                    $this->Flash->success(
                        __(
                            "Account information of selected user has been updated successfully."
                        )
                    );
                    ...
                    $this->Flash->error(
                        __(
                            "Entred username does not exist in our database. Please enter the correct username for selected member."
                        )
                    );
                }
            } else {
    ...
    public function editProfile($intUserId = null)
    {
        if (!$this->request->getSession()->check("adminUserId")) {
            return $this->redirect($this->backend_url.'/user/login');
        }
        ...
        $join = [
            [
                ...
                "conditions" => [
                    'Photo.reference_id = Users.id AND Photo.reference_type = "user_photo"',
                ],
            ],
            ...
            [
                ...
                "conditions" => [
                    'IdProof.reference_id = Users.id AND IdProof.reference_type = "id_proof"',
                ],
            ],
            ...
            [
                ...
                "conditions" => [
                    'AddressProof.reference_id = Users.id AND AddressProof.reference_type = "address_proof"',
                ],
            ],
        ];
        ...
        $conditions = [
            "States.id" => $userInfo->detail->state_id ? $userInfo->detail->state_id : '',
        ];
        ...
        if ($this->request->is("post")) {
            ...
            $username = $this->request->getData()["User"]["username"]
                ? trim($this->request->getData()["User"]["username"])
                : "";

            $contact_no = $this->request->getData()["Detail"]["contact_no"]
                ? $this->request->getData()["Detail"]["contact_no"]
                : "";

            if (!empty($username) && !empty($contact_no)) {
                $checkUsername = $usersTable
                    ->find("all", [
                        "conditions" => [
                ...
                /*elseif($checkContactNumber > 0){
                    ...
                    if ($checkPanCount < 1000) {
                        $user_id = $this->request->getData()["User"]["id"];
                        ...
                        if ($usersTable->save($userData)) {
                            ...
                            if (!empty($detail)) {
                                $detailData = $detailsTable->get($detail->id);
                                ...
                                $detailData->dob =
                                    $this->request->getData()["Detail"]["dob"];
                                ...
                                if (
                                    !empty($this->request->getData()["Attachment"])
                                ) {
                                    foreach (
                                        ...
                                        $attachmentData->reference_id = $user_id;
                            ...
                            $this->Flash->success(
                                __("User has been updated successfully.")
                            );
                            return $this->redirect($this->backend_url.'/users/index');
                        }
                    } else {
    ...
    public function userDashboard()
    {
        ...
        $title = $prefix_title . " User : Dashboard";
        ...
        $fields = ["Binaries.amount", "Binaries.percentage"];
            ...
            $query = $conn->execute(
                "SELECT SUM(Payouts.direct_amount) as directIncome, SUM(Payouts.matching_amount) as matchingAmount, SUM(Payouts.royalty_amount) as royaltyAmount, SUM(Payouts.roi) as roiAmount, u.is_mobile_club, u.is_laptop_club, u.is_bike_club,
                    ...
                    "') AS totalUpgradesAmount,
                                ...
                                (SELECT (SUM(pNetAmt.net_amount) - SUM(pNetAmt.admin_commission)) FROM payments as pNetAmt WHERE pNetAmt.user_id = '" .
                    $user_id .
                    "') AS totalPayouts,
                    ...
                    "' AND pWithdrawals.status = '0') AS pendingWithdrawls,
                                ...
                                (SELECT SUM(tPackages.package_amount) FROM upgrades as tPackages WHERE tPackages.upgraded_id = '" .
                    $user_id .
                    ...
                    "') AS totaPendingPayable
                                ...
                                INNER JOIN users u ON Payouts.upagraded_user_id = u.id



                                WHERE Payouts.upagraded_user_id = '" .
                    $this->user->id .
                    "'
    ...
    public function addUser()
    {
        ...
        $title = $prefix_title . " Manage Users : Add";
        ...
        if ($this->request->is("post")) {
            ...
            $epin = isset($postData["User"]["epin"])
                ? trim($postData["User"]["epin"])
                : null;

            $rank = isset($postData["User"]["rank"])
                ? trim($postData["User"]["rank"])
                : null;

            $plot_id = isset($postData["User"]["plot_id"])
                ? trim($postData["User"]["plot_id"])
                : null;

            $email = isset($postData["User"]["email"])
                ? trim($postData["User"]["email"])
                : null;

            $sponser_username = isset(
                $postData["User"]["sponser_username"]
            )
                ? trim($postData["User"]["sponser_username"])
                : null;

            $position = isset($postData["User"]["position"])
                ? trim($postData["User"]["position"])
                : null;

            $first_name = isset($postData["Detail"]["first_name"])
                ? trim($postData["Detail"]["first_name"])
                : "";

            $last_name = isset($postData["Detail"]["last_name"])
                ? trim($postData["Detail"]["last_name"])
                : "";

            $contact_no = isset($postData["Detail"]["contact_no"])
                ? trim($postData["Detail"]["contact_no"])
                : "";

            $photo_attachment_id = isset(
                $postData["Detail"]["photo_attachment_id"][0]
            )
                ? $postData["Detail"]["photo_attachment_id"][0]
                : null;

            $address_attachment_id = isset(
                $postData["Detail"]["address_attachment_id"][0]
            )
                ? $postData["Detail"]["address_attachment_id"][0]
                : null;

            $pan_attachment_id = isset(
                $postData["Detail"]["pan_attachment_id"][0]
            )
                ? $postData["Detail"]["pan_attachment_id"][0]
                : null;
            ...
            if (
                ...
                //echo"<pre>";print_r($checkUsername);exit;

                /*if($checkEmail >= 700){

                    $this->Flash->error(__('Entered email id already used 7 times by our registered user. So Please resgister with different email id.'));
                ...
                /*elseif($checkContactNumber >= 7){
                   ...
                   $this->Flash->error(__('Entered contact number already used 7 times by our registered user. Please resgister with different contact number'));
                    ...
                    $user->parent_name =$getLastUserInfo->Details->first_name ." " . $getLastUserInfo->Details->last_name;
                    $user->sponsor_id = $sponserInfo->id;
                    $user->sponsor_name = $sponserInfo->Details["first_name"] ." " .$sponserInfo->Details["last_name"];
                    $user->position = $position;
                    $user->email = $email;
                    $user->username = $username;
                    $user->password = md5($password);
                    $user->transaction_password = md5(12345);
                    $user->total_left = 0;
                    $user->total_right = 0;
                    ...
                    if ($usersTable->save($user)) {
                        ...
                        $detail->first_name = isset(
                            $postData["Detail"]["first_name"]
                        )
                            ? $postData["Detail"]["first_name"]
                            : null;

                        $detail->last_name = isset(
                            $postData["Detail"]["last_name"]
                        )
                            ? $postData["Detail"]["last_name"]
                            : null;

                        $detail->pan_number = isset(
                            $postData["Detail"]["pan_number"]
                        )
                            ? $postData["Detail"]["pan_number"]
                            : null;

                        $detail->adhar_number = isset(
                            $postData["Detail"]["adhar_number"]
                        )
                            ? $postData["Detail"]["adhar_number"]
                            : null;

                        $detail->country_id = isset(
                            $postData["Detail"]["country_id"]
                        )
                            ? $postData["Detail"]["country_id"]
                            : null;

                        $detail->contact_no = isset(
                            $postData["Detail"]["contact_no"]
                        )
                            ? $postData["Detail"]["contact_no"]
                            : null;
                        ...
                        $detail->bank_name = isset(
                            $postData["Detail"]["bank_name"]
                        )
                            ? $postData["Detail"]["bank_name"]
                            : null;

                        $detail->account_number = isset(
                            $postData["Detail"]["account_number"]
                        )
                            ? $postData["Detail"]["account_number"]
                            : null;

                        $detail->ifsc_code = isset(
                            $postData["Detail"]["ifsc_code"]
                        )
                            ? $postData["Detail"]["ifsc_code"]
                            : null;
                        ...
                        $detail->phone_pay_number = isset(
                            $postData["Detail"]["phone_pay_number"]
                        )
                            ? $postData["Detail"]["phone_pay_number"]
                            : null;

                        $detail->paytm_number = isset(
                            $postData["Detail"]["paytm_number"]
                        )
                            ? $postData["Detail"]["paytm_number"]
                            : null;

                        $detail->is_kyc_approved = isset(
                            $postData["Detail"]["is_kyc_approved"]
                        )
                            ? $postData["Detail"]["is_kyc_approved"]
                            : null;
                        ...
                        //echo"<pre>";print_r($getLastUserInfo);exit;
                        ...
                        $Email->template('registration', 'emaillayout')
                              ...
                              ->subject('Dear '.$postData['Detail']['first_name'].' '.$postData['Detail']['last_name'].' ! You have successfully registered on admireglobal.io.')
                        ...
                        $template =
                            ...
                            ", Welcome to Fashioholic, Your Username: " .
                            $username .
                            " Password: " .
                            $password .
                            " Please visit fashion.fashioholic.biz";

                       // $sendSMS = $usersTable->sendSMS($template, $contact_no);

                        $msg = 'Dear '.$postData["Detail"]["first_name"].', Welcome to FashionHolic,  Your usernmae='.$username.' and Password='.$password.' Please visit fashion.fashioholic.biz';
    ...
    public function userAdded()
    {
        ...
        $title = $prefix_title . " Manage Users : User Added";

        $this->set("title", $title);
    }

    public function userEmi($intUserId = null)
    {
        if (!$this->request->getSession()->check("adminUserId")) {
            return $this->redirect($this->backend_url.'/user/login');
        }
        ...
        $title = $prefix_title . " User : Emi";

        $this->set("title", $title);

        $usersTable = TableRegistry::get("Users");

        $userPropertyMappingsTable = TableRegistry::get("UserPropertyMappings");
        ...
        if (!empty($intUserId)) {
            $conditions = [
                "UserPropertyMappings.user_id" => $intUserId,
            ];

            $emis = $userPropertyMappingsTable
                ->find("all", ["conditions" => $conditions])
                ->enableAutoFields(true)
                ->toArray();
        ...
        if ($this->request->is("post")) {
            ...
            $id = isset($this->request->getData()["UserPropertyMapping"]["id"])
                ? $this->request->getData()["UserPropertyMapping"]["id"]
                : "";

            $amount = isset(
                $this->request->getData()["UserPropertyMapping"]["amount"]
            )
                ? $this->request->getData()["UserPropertyMapping"]["amount"]
                : "";

            $direct_income_percentage = isset(
                $this->request->getData()["UserPropertyMapping"][
            ...
            if (
                !empty($id) &&
                !empty($amount) &&
                !empty($direct_income_percentage)
            ) {
                ...
                $userPropertyMappingsData = $userPropertyMappingsTable->get(
                    $id
                );

                $userPropertyMappingsData->status = 2;

                if (
                    $userPropertyMappingsTable->save($userPropertyMappingsData)
                ) {
                    $conditions = [
                        "UserPropertyMappings.id" => $id,
                    ];

                    $userPropertyMappingInfo = $userPropertyMappingsTable
                        ->find("all", ["conditions" => $conditions])
                        ->first();

                    /*echo '<pre>';

                    print_r($userPropertyMappingInfo);exit;*/

                    $usersTable->updateParentsOnPayEmi(
                        $intUserId,
                        $userInfo->parent_id,
                        $userPropertyMappingInfo->matching_amount
                    );

                    if (!empty($userInfo->sponsor_id)) {
                        $sponsorInfo = $usersTable
                            ->find("all", [
                                "conditions" => [
                        ...
                        $sponsor = $usersTable->get($userInfo->sponsor_id);
                        ...
                        $directAmount = $userPropertyMappingInfo->direct_amount;

                        $payout = $payoutsTable->newEmptyEntity();

                        $payout->upagraded_user_id = $userInfo->sponsor_id;
            ...
            $this->Flash->success(__("EMI has been paid successfully."));

            return $this->redirect(
                $this->backend_url . "/user/user-emi/" . $intUserId
            );
        }
    }
    ...
    public function bulkupload()
    {
        if ($this->request->is("post")) {
            ...
            $object = bulk_upload($this->request->getData());

            foreach ($object->getWorksheetIterator() as $worksheet) {
                print_r($worksheet->getCellByColumnAndRow(3, 1)->getValue());
    ...
    public function currentRate()
    {
        ...
        $title = $prefix_title . " Current Rate";
        ...
        $currentRates = $currentRatesTable
            ->find("all", ["conditions" => $conditions])
            ->enableAutoFields(true)
            ->toArray();

        $this->set("currentRates", $currentRates);
    }

    public function addCurrentRate()
    {
        if (!$this->request->getSession()->check("adminUserId")) {
            return $this->redirect($this->backend_url.'/user/login');
        }
        ...
        if ($this->request->is("post")) {
            ...
            $plan = isset($this->request->getData()["CurrentRate"]["plan"])
                ? trim($this->request->getData()["CurrentRate"]["plan"])
                : "";

            $rate = isset($this->request->getData()["CurrentRate"]["rate"])
                ? trim($this->request->getData()["CurrentRate"]["rate"])
                : "";

            $remark = isset($this->request->getData()["CurrentRate"]["remark"])
                ? trim($this->request->getData()["CurrentRate"]["remark"])
                : "";

            if (!empty($plan) && !empty($rate) && !empty($remark)) {
                $currentRatesTable->updateAll(
                    ["status" => 0],
                    ["plan" => $plan, "status" => 1]
                );

                $currentRateData = $currentRatesTable->newEmptyEntity();

                $currentRateData->plan = $plan;

                $currentRateData->rate = $rate;

                $currentRateData->remark = $remark;

                $currentRateData->status = 1;

                if ($currentRatesTable->save($currentRateData)) {
                    $this->Flash->success(
                        __(
                            "Congratulations! Current Rate has been added successfully."
                        )
                    );

                    return $this->redirect(
                        $this->backend_url . "/user/current-rate"
                    );
                }
            }
        }
    ...
    public function assignPlot()
    {
        ...
        $title = $prefix_title . " Assign Plot";
        ...
        $currentRateInfo = $currentRatesTable
            ->find("all", ["conditions" => $conditions])
            ->first();

        $this->set("currentRateInfo", $currentRateInfo);
        ...
        if ($this->request->is("post")) {
            ...
            $user_id = isset($this->request->getData()["AssignPlot"]["user_id"])
                ? trim($this->request->getData()["AssignPlot"]["user_id"])
                : "";

            $property_id = isset(
                $this->request->getData()["AssignPlot"]["property_id"]
            )
                ? trim($this->request->getData()["AssignPlot"]["property_id"])
                : "";

            $site_id = isset($this->request->getData()["AssignPlot"]["site_id"])
                ? trim($this->request->getData()["AssignPlot"]["site_id"])
                : "";
            ...
            $plot_id = isset($this->request->getData()["AssignPlot"]["plot_id"])
                ? trim($this->request->getData()["AssignPlot"]["plot_id"])
                : "";

            $plot_number = isset(
                $this->request->getData()["AssignPlot"]["plot_number"]
            )
                ? trim($this->request->getData()["AssignPlot"]["plot_number"])
                : "";

            $length = isset($this->request->getData()["AssignPlot"]["length"])
                ? trim($this->request->getData()["AssignPlot"]["length"])
                : "";

            $width = isset($this->request->getData()["AssignPlot"]["width"])
                ? trim($this->request->getData()["AssignPlot"]["width"])
                : "";

            $area = isset($this->request->getData()["AssignPlot"]["area"])
                ? trim($this->request->getData()["AssignPlot"]["area"])
                : "";

            $location = isset($this->request->getData()["AssignPlot"]["location"])
                ? trim($this->request->getData()["AssignPlot"]["location"])
                : "";

            $east = isset($this->request->getData()["AssignPlot"]["east"])
                ? trim($this->request->getData()["AssignPlot"]["east"])
                : "";

            $west = isset($this->request->getData()["AssignPlot"]["west"])
                ? trim($this->request->getData()["AssignPlot"]["west"])
                : "";

            $north = isset($this->request->getData()["AssignPlot"]["north"])
                ? trim($this->request->getData()["AssignPlot"]["north"])
                : "";

            $south = isset($this->request->getData()["AssignPlot"]["south"])
                ? trim($this->request->getData()["AssignPlot"]["south"])
                : "";

            $plan = isset($this->request->getData()["AssignPlot"]["plan"])
                ? trim($this->request->getData()["AssignPlot"]["plan"])
                : "";

            $plc = isset($this->request->getData()["AssignPlot"]["plc"])
                ? trim($this->request->getData()["AssignPlot"]["plc"])
                : "";

            $edc = isset($this->request->getData()["AssignPlot"]["edc"])
                ? trim($this->request->getData()["AssignPlot"]["edc"])
                : "";

            $ifmc = isset($this->request->getData()["AssignPlot"]["ifmc"])
                ? trim($this->request->getData()["AssignPlot"]["ifmc"])
                : "";

            $bsp = isset($this->request->getData()["AssignPlot"]["bsp"])
                ? trim($this->request->getData()["AssignPlot"]["bsp"])
                : "";

            $current_rate = isset(
                $this->request->getData()["AssignPlot"]["current_rate"]
            )
                ? trim($this->request->getData()["AssignPlot"]["current_rate"])
                : "";

            $total_amount = isset(
                $this->request->getData()["AssignPlot"]["total_amount"]
            )
                ? trim($this->request->getData()["AssignPlot"]["total_amount"])
                : "";

            $plc_amount = isset(
                $this->request->getData()["AssignPlot"]["plc_amount"]
            )
                ? trim($this->request->getData()["AssignPlot"]["plc_amount"])
                : "";

            $grand_total = isset(
                $this->request->getData()["AssignPlot"]["grand_total"]
            )
                ? trim($this->request->getData()["AssignPlot"]["grand_total"])
                : "";

            $discount = isset($this->request->getData()["AssignPlot"]["discount"])
                ? trim($this->request->getData()["AssignPlot"]["discount"])
                : "";

            $remark = isset($this->request->getData()["AssignPlot"]["remark"])
                ? trim($this->request->getData()["AssignPlot"]["remark"])
                : "";
            ...
            if (
                ...
                $conditions = [
                    "AssignPlots.property_id" => $property_id,

                    "AssignPlots.site_id" => $site_id,

                    "AssignPlots.block_id" => $block_id,

                    "AssignPlots.plot_id" => $plot_id,

                    "AssignPlots.status !=" => 3,
                ];

                $assignPlotInfo = $assignPlotsTable
                    ->find("all", ["conditions" => $conditions])
                    ->first();

                if (empty($assignPlotInfo)) {
                    $assignPlotData = $assignPlotsTable->newEmptyEntity();

                    $assignPlotData->user_id = $user_id;

                    $assignPlotData->property_id = $property_id;

                    $assignPlotData->site_id = $site_id;

                    $assignPlotData->block_id = $block_id;

                    $assignPlotData->plot_id = $plot_id;

                    $assignPlotData->plot_number = $plot_number;

                    $assignPlotData->length = $length;

                    $assignPlotData->width = $width;

                    $assignPlotData->area = $area;

                    $assignPlotData->location = $location;

                    $assignPlotData->east = $east;

                    $assignPlotData->west = $west;

                    $assignPlotData->north = $north;

                    $assignPlotData->south = $south;

                    $assignPlotData->plc = $plc;

                    $assignPlotData->edc = $edc;

                    $assignPlotData->ifmc = $ifmc;

                    $assignPlotData->bsp = $bsp;

                    $assignPlotData->plan = $plan;

                    $assignPlotData->current_rate = $current_rate;

                    $assignPlotData->total_amount = $total_amount;

                    $assignPlotData->plc_amount = $plc_amount;

                    $assignPlotData->grand_total = $grand_total;

                    $assignPlotData->discount = $discount;

                    $assignPlotData->remark = $remark;

                    $assignPlotData->status = 1;

                    if ($assignPlotsTable->save($assignPlotData)) {
                        $plotData = $plotsTable->get($plot_id);
                        ...
                        $this->Flash->success(
                            __(
                                "Congratulations! Plot has been assigned successfully."
                            )
                        );

                        return $this->redirect(
                            $this->backend_url . "/user/assigned-plots"
                        );
                    }
                } else {
                    $this->Flash->error(
                        __("Selected plots already has been assigned.")
                    );
                }
            } else {
    ...
    public function assignedPlots()
    {
        ...
        $title = $prefix_title . " Assigned Plots";
        ...
        $join = [
            [
                ...
                "conditions" => ["Users.id = AssignPlots.user_id"],
            ],
    ...
    public function changeAssignedPlotStatus($intStatus, $intId)
    {
        if (!$this->request->getSession()->check("adminUserId")) {
            return $this->redirect($this->backend_url.'/user/login');
        }
        ...
        $title = $prefix_title . " Change Status";
        ...
        if (
            empty($intStatus) ||
            (empty($intId) && !is_numeric($intStatus) && !is_numeric($intId))
        ) {
            return $this->redirect($this->backend_url . "/user/assigned-plots");
        }

        $assignPlotData = $assignPlotsTable->get($intId);

        $assignPlotData->status = $intStatus;

        if ($assignPlotsTable->save($assignPlotData)) {
            $conditions = [
                "AssignPlots.id" => $intId,
            ];

            $assginedPlotInfo = $assignPlotsTable
                ->find("all", ["conditions" => $conditions])
                ->first();
            ...
            if (!empty($assginedPlotInfo)) {
                ...
                $plotData = $plotsTable->get($assginedPlotInfo->plot_id);
            ...
            $this->Flash->success(
                __("Congratulations! Status has been changed successfully.")
            );
    ...
    public function kyc($intUserId = null)
    {
        if (!$this->request->getSession()->check("adminUserId")) {
            return $this->redirect($this->backend_url.'/user/login');
        }
        ...
        if ($this->request->is("post")) {
            ...
            $detailData->is_kyc_approved = isset(
            ...
            if ($detailsTable->save($detailData)) {
                ...
                $this->Flash->success(
                    __("Congratulations! KYC details has updated successfully.")
                );

                return $this->redirect(
                    $this->backend_url . "/user/kyc/" . $userId
                );
            }
    ...
    public function assignedPlotDetails($intAssignedPlotId = null)
    {
        if (!$this->request->getSession()->check("adminUserId")) {
            return $this->redirect($this->backend_url.'/user/login');
        }
        ...
        $title = $prefix_title . " Assigned Plot Details";

        $this->set("title", $title);

        if (empty($intAssignedPlotId)) {
            return $this->redirect($this->backend_url . "/user/assigned-plots");
        }
        ...
        $conditions = [
            "AssignPlots.id" => $intAssignedPlotId,
        ];
        ...
        $assignedPlotInfo = $assignPlotsTable
            ->find("all", [
                "fields" => $fields,
                "join" => $join,
                "conditions" => $conditions,
        ...
        $this->set("assignedPlotInfo", $assignedPlotInfo);
    }

    public function viewUser($encUserId)
    {
        if (!$this->request->getSession()->check("adminUserId")) {
            return $this->redirect($this->backend_url.'/user/login');
        }
        ...
        $title = $prefix_title . " User : View User";
        ...
        $userInfo = $usersTable
            ->find("all", ["conditions" => ["Users.id" => $userId]])
            ->first();

        if (empty($userInfo)) {
            return $this->redirect($this->backend_url . "/users");
        }

        $this->request->getSession()->write("userId", $userInfo->id);
