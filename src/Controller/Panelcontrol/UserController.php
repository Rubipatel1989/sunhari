...
class UserController extends AppController
{
    ...
    public function dashboard()
    {
        ...
        $fundrequests = $fundrequestsTable->find('all', array('fields' => $fields, 'conditions' => $conditions, 'order' => $order, 'join' => $join, 'limit' => $limit))->enableAutoFields(true)->toArray();
    ...
    public function upgrade()
    {
        ...
        //$usersTable->updateAchievedRewards(510);
        ...
        if ($this->request->is("post")) {
            ...
            if (
                ...
                if (empty($package)) {
                    ...
                    $this->Flash->error(
                        __(
                            "Sorry! selected user can not be upgraded because user has already upgraded."
                        )
                    );
                } elseif (
                    !empty($upgradeInfo) &&
                    ...
                    if ($upgradesTable->save($upgrade)) {
                        ...
                        if ($usersTable->save($userInfo)) {
                            ...
                            //$usersTable->updateParentsOnUpgrade($upgraded_id, $userData->parent_id, $package_bv);
                            ...
                            /*$usersTable->updateAchievedRewards($upgraded_id, $upgraded_id);



                            $usersTable->updateAchievedBonanzas($upgraded_id, $upgraded_id);*/

                            $this->Flash->success(
                                __("User has been upgraded successfully.")
                            );

                            return $this->redirect($this->backend_url.'/user/upgrade');
                        }
                    }
                }
            }
    ...
    public function degrade()
    {
        ...
        if ($this->request->is("post")) {
            ...
            if (isset($this->request->getData()["Upgrade"]["upgraded_id"])) {
                $user_id = $this->adminUser->id;
                ...
                if (!empty($upgradeInfo)) {
                    ...
                    return $this->redirect($this->backend_url.'/user/degrade');
                }
            }
        }
    }
        ...
        if ($this->request->is("post")) {
            ...
            if (
                ...
                if ($checkUsername > 0) {
                    ...
                    return $this->redirect($this->backend_url.'/user/edit-account/'.$intUserId);
                } else {
                    $this->Flash->error(
                        __(
    ...
    public function editProfile($intUserId = null)
    {
        ...
        $conditions = [
            "States.id" =>  isset($userInfo->detail->state_id) ? $userInfo->detail->state_id : ''
        ];
                ...
                /*elseif($checkContactNumber > 0){
                    ...
                    if ($checkPanCount < 1000) {
                        ...
                        if ($usersTable->save($userData)) {
                            ...
                            return $this->redirect($this->backend_url.'/user/edit-profile/'.$intUserId);
                        }
                    } else {
                        $this->Flash->error(
                            __(
    ...
    public function oldJoining()
    {
        ...
        if ($this->request->is("post")) {
            ...
            $created = isset($this->request->getData()["User"]["joining_date"]) ? trim($this->request->getData()["User"]["joining_date"]) : null;
