  ...
  <form name="ajax-upload-form" id="ajax-upload-form" action="<?php echo $home_url; ?>/ajax/upload_file" method="post" enctype="multipart/form-data" class="hidden">
    <?php echo $this->Form->input('attachment_file', array('type' => 'file', 'div' => false, 'label' => false, 'class'=>'id_proof_upload')); ?>
  </form>
<script type="text/javascript">
  jQuery(document).ready(function(){
    jQuery(".id-proof").click(function(){
      jQuery(".id_proof_upload").click();
    });
    jQuery(".id_proof_upload").change(function(){
      var _this = this;
      ...
      jQuery.ajax({
        type: "POST",
        url: "<?php echo $home_url; ?>/ajax/id_proof_upload",
        ...
        success: function(responseText){
          ...
          if(responseText == 0){
            ...
            jQuery('.id-proof-container').html(responseText);
            jQuery(_this).val('');
          }
          jQuery(".body-loader").hide();
        }
